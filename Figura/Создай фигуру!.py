...
class MainWindow:
    def __init__(self, master):
        self.master = master
        self.master.title("Нарисуй мне фигуру!")
        self.master.resizable(False, False)
        self.width, self.height = self.master.winfo_screenwidth() // 2 - 250, self.master.winfo_screenheight() // 2 - 268
        self.master.geometry('500x536+{}+{}'.format(self.width, self.height))

        self.canvas = Canvas(width=500, height=500, bg="white")
        window_create = Button(width=40, height=1, text="Создать фигуру",
...
class FigureWindow:
    def __init__(self, main_window, w_h_array): # Создаём окно
        ...
        self.colors = ["black", "white", "snow", "linen", "lavender", "misty rose", "gray", "blue", "cyan", "yellow", "gold",
    ...
    def window_content(self, window): # Заполняем окно
        ...
        draw_button = Button(frame_global, text="Нарисовать фиругу",
            command= lambda: self.create_figure((self.x1_entry, self.y1_entry, self.x2_entry, self.y2_entry)))
    ...
    def create_figure(self, elements):
        ...
        if self.typeVar.get():
            self.main.canvas.create_oval((x1, y1), (x2, y2), fill=self.color_fill.get(0), outline=self.color_outline.get(0))
        else:
            self.main.canvas.create_rectangle((x1, y1), (x2, y2), fill=self.color_fill.get(0), outline=self.color_outline.get(0))
        self.window.destroy()
